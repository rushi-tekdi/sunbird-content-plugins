...
var fileUploader;
angular
  .module("org.ekstep.uploadcontent-1.6", [])
    ...
    function ($scope, $injector, instance) {
      ...
      $scope.$on("ngDialog.opened", function () {
        ...
        $scope.uploader = new qq.FineUploader({
          ...
          multiple: false,
          validation: {
            allowedExtensions: ["pdf", "epub", "mp4", "h5p", "zip", "webm", "mp3", "wav"],
            itemLimit: 1,
      ...
      $scope.upload = function () {
        ...
        if (!mimeType) {
          ecEditor.dispatchEvent("org.ekstep.toaster:error", {
            message:
              "Invalid content type (supported type: mp3, wav, pdf, epub, h5p, mp4, youtube, html-zip, webm, whitelisted-domain)",
            position: "topCenter",
            icon: "fa fa-warning",
          });
          $scope.showLoader(false);
      ...
      $scope.checkUploadApi = function (fileURL) {
        const apiData = {
          contenturl: fileURL,
        };

        fetch("/api/content-upload/get-status", {
          method: "POST",
          headers: {
          ...
          .then((response) => {
            if (!response.ok) {
              ...
              $scope.showLoader(false);
              $scope.checkapiload = false;
            }
            return response.json();
          })
          .then((data) => {
            console.log("######## zip fix data", data);
      ...
      $scope.uploadByURL = function (fileUpload, mimeType) {
        var cb = function (fileURL) {
          ...
          console.log("############# i am in fileURL", fileURL);
          console.log("############# i am in mimeType", mimeType);
          var config = {
            ...
            function (err, res) {
              if (err) {
                if (
                  ...
                  setInterval(() => {
                    if ($scope.checkapiload) {
                      $scope.checkUploadApi(fileURL);
                    }
                ...
                if (
                  ...
                  /*var timeout = Number(
                    ecEditor.getConfig("uploadDelayTimeout")
                  );
