...
var fileUploader;
angular
  .module("org.ekstep.uploadlargecontent-1.1", [])
  .controller("largeUploadController", [
    ...
    function ($scope, $injector, instance) {
      ...
      $scope.brandingName = ecEditor.getConfig("branding");
      ...
      $scope.$on("ngDialog.opened", function () {
        ...
        $scope.uploader = new qq.FineUploader({
          ...
          messages: {
            sizeError:
              "{file} You are trying to upload a file that exceeds the maximum size allowed : " +
              $scope.humanReadableFileSize($scope.maxUploadSize),
            minSizeError:
              "{file} is too small, minimum file size is " +
              $scope.humanReadableFileSize($scope.minUploadSize),
            typeError:
              "{file} The format of the file you are uploading is not supported. You can upload only {extensions} files formats",
            onLeave:
              "{file} is being uploaded, if you leave now the upload will be cancelled",
          },
          callbacks: {
      ...
      $scope.fileValidation = function () {
        ...
        $scope.mimeType =
          $scope.uploader.getFile(0) != null
            ? $scope.detectMimeType($scope.uploader.getName(0))
            : "";
        if (!$scope.mimeType) {
          $scope.toasterMsgHandler(
            "error",
            "Invalid content type (supported type: mp4, webm)"
          );
          return;
        } else {
          $scope.createContent();
        ...
        $scope.contentService.getPresignedURL(
          ...
          function (err, res) {
            ...
            if (err) {
              ...
              console.log(
                "getPresignedURL pre_signed_url.." +
                  res.data.result.pre_signed_url
              );
              var cloudstorage = _.get(
                ecEditor.getConfig("cloudStorage"),
                "provider"
              );
              ...
              if (cloudstorage != null || typeof cloudstorage != "undefined") {
                ...
                $scope.uploaderLib
                  ...
                  .on("error", (error) => {
                    ...
                    $scope.toasterMsgHandler(
                      "error",
                      "The upload seems to have failed. Click Retry to resume uploading the video."
                    );
                  ...
                  .on("progress", (progress) => {
                    ...
                    console.log("############## progress outer", progress);
                ...
                config =
                  $scope.contentService.appendCloudStorageHeaders(config);
                $scope.contentService.uploadDataToSignedURL(
                  ...
                  function (err, res) {
                    if (err) {
                      ...
                      $scope.updateContentWithURL(
                        $scope.submitUri.split("?")[0]
                      );
      ...
      $scope.uploadFileInBlocks = function () {
        ...
        if ($scope.totalBytesRemaining > 0) {
          var fileContent = $scope.selectedFile.slice(
            $scope.currentFilePointer,
            $scope.currentFilePointer + $scope.maxBlockSize
          );
          var blockId =
            $scope.blockIdPrefix + $scope.pad($scope.blockIds.length, 6);
      ...
      $scope.commitBlockList = function () {
        ...
        const blockListPromise = $scope.fetchRetry(
          uri,
          {
            headers: {
              "content-type":
          ...
          $scope.delayBetweenRetryCalls,
          $scope.retryChunkUploadLimit
        );

        blockListPromise
          .then($scope.handleErrors)
          .then(function () {
            $(".progress").progress({
              percent: 100,
              text: {
      ...
      $scope.reader.onloadend = function (evt) {
        if (evt.target.readyState == FileReader.DONE) {
          var uri =
            $scope.submitUri +
            "&comp=block&blockid=" +
            $scope.blockIds[$scope.blockIds.length - 1];
          var requestData = new Uint8Array(evt.target.result);
          const fetchPromise = $scope.fetchRetry(
            uri,
            {
              headers: {
            ...
            .then(function (response) {
              if (response.ok) {
                ...
                $scope.percentComplete = (
                  (parseFloat($scope.bytesUploaded) /
                    parseFloat($scope.selectedFile.size)) *
                  100
                ).toFixed(2);
      ...
      $scope.fileUploadingTimeCalculation = function () {
        ...
        var uploadSpeed = Math.floor(
          $scope.bytesUploaded / (timeElapsed / 1000)
        ); // Upload speed in second
        var estimatedSecondsLeft = Math.round(
          ($scope.selectedFile.size - $scope.bytesUploaded) / uploadSpeed
        );
